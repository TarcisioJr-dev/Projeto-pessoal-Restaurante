...
function somar(){
  ...
  if((cafepreto==0)&&(cafeleite==0)&&(cafecreme==0)&&(bolo==0)&&(pao==0)&&(donut==0)&&(misto==0)&&(frango==0)&&(peixe==0)&&(vaca==0)&&(porco==0)&&(coca==0)&&(guara==0)&&(sprite==0)&&(laranja==0)&&(acerola==0)&&(skol==0)&&(brahma==0)&&(heine==0)&&(corona==0)&&(vodka==0)&&(wisky==0)&&(fritas==0)&&(fritasbac==0)&&(carneassada==0)&&(espetinho==0)&&(xtudo==0)){
    cupom = '<p>Nenhuma compra foi realizada!</p><p>Espero que na próxima encontre o que deseja!</p><p id="emoji">\u{1F601}</p>'
    cupom += '<button onclick="fechar()">Fechar</button>'
    res.innerHTML = cupom
  }else{
    compras = ''
    total = 0
    cupom = '<p>A compra feita foi:</p>'
    if(cafepreto > 0){
    ...
    cupomfinal = '<p>As compras feitas foram:</p>'
    cupomfinal += compras
    cupomfinal += `<p>O valor total da compra foi R$${total.toFixed(2)}</p>`
    cupomfinal += '---------------------------------------'

    cupom += compras
    cupom += `<p>O valor total da compra foi R$${total.toFixed(2)}`
    somatotal = Number(total)
    cupom += `<br>Deseja finalizar a compra ou comprar mais?</p>`
    cupom += `<br>`
    cupom += '<p><button id="pagar" onclick="pagar()">Pagar</button>   '
    cupom += '<button id="comprar" onclick="fechar()">Comprar</button></p>'
    res.innerHTML = cupom
    
    cupom = ''
    cupom += `<p>O valor total da compra foi R$${total.toFixed(2)}</p><br>`
  }
...
function pagar(){
  cupom += '<p>Informe o valor para pagamento:</p>'
  cupom += '<p>R$<input type="number" name="vlrpago" id="vlrpago" autofocus min="0">  '
...
function processarpagamento(){
  ...
  if(vlrpago == 0){
    ...
    if(restante > 0){
      cupom = `<p>O valor restante para pagamento é R$${restante.toFixed(2)}</p>`
      pagar()
    }else{
      cupomfinal += '<br>---------------------------------------'
      cupomfinal += '<p>+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+</p>'
      cupomfinal += `<p>O valor total pago foi R$${somatroco.toFixed(2)}</p>`
      cupomfinal += `<p>O seu troco é: R$${-restante.toFixed(2)}</p>`
      cupomfinal += '<p>Obrigado por comprar conosco! \u{1F601}</p>'
      cupomfinal += '---------------------------------------'
      cupomfinal += `<p><button onclick="finalizar()">Finalizar</button>`
